...
class Controller extends Cloud {
	...
	public function get_app_info( $args = array() ) {
		...
		if ( 200 === $response_code ) {
			...
			$scan_timestamp  = isset( $response['last_scan_at'] ) ? strtotime( sanitize_text_field( $response['last_scan_at'] ) ) : '';
			$date            = gmdate( 'd M Y', $scan_timestamp );
			$time            = gmdate( 'H:i:s', $scan_timestamp );
